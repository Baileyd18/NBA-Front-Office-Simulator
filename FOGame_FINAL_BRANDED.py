...
def calculate_team_metrics(roster_df: pd.DataFrame, salary_cap: int) -> dict:
    ...
    overall_score = (
        ...
        depth_score * 0.10 +
        shooting_score * 0.06 +
        rebounding_score * 0.04 +
        fit_score * 0.03 +
        versatility_score * 0.01
    )

    # Less harsh curve: solid playoff rosters can reach the 50s/60s,
    # elite teams reach the 70s, historic builds threaten 82.
    projected_wins = round(16 + (overall_score / 100) * 66)
    ...
    if payroll > salary_cap:
        ...
        projected_wins -= penalty
        overall_score -= penalty * 0.40
...
with st.sidebar:
    ...
    sidebar_open_slots = [slot for slot in ROSTER_SLOTS if slot not in st.session_state.roster]
    ...
    if len(sidebar_open_slots) == 0:
        ...
        sidebar_selected_display = st.selectbox(
            "Player",
            sidebar_available_players["Display"].tolist(),
            key="sidebar_quick_player"
        )

        if st.button("Add Player", type="primary", key="sidebar_add_player"):
            sidebar_selected_player = sidebar_available_players[
                sidebar_available_players["Display"] == sidebar_selected_display
            ].iloc[0].to_dict()
...
with left:
    ...
    st.markdown(
        '<div class="draft-board-note">Draft board view: core box-score production, salary, and shooting splits.</div>',
        unsafe_allow_html=True
    )

    draft_board = player_pool[
        [
            "Player",
            "Pos",
            "Salary Display",
    ...
    st.markdown('<div class="section-title">Add Player</div>', unsafe_allow_html=True)
    ...
    if len(available_players) == 0:
        ...
        selected_display = st.selectbox(
            "Select a player",
            available_players["Display"].tolist()
        )

        open_slots = [slot for slot in ROSTER_SLOTS if slot not in st.session_state.roster]
        ...
        if st.button("Add to Roster", type="primary"):
            if selected_slot == "Roster Full":
                ...
                selected_player = available_players[available_players["Display"] == selected_display].iloc[0].to_dict()
                ...
                # Clear reports because roster changed.
                st.session_state.reports = {}
                st.rerun()
...
with right:
    st.markdown('<div class="section-title">Your 13-Man Roster</div>', unsafe_allow_html=True)
    ...
    for slot in ROSTER_SLOTS:
        if slot in st.session_state.roster:
            ...
            fit, notes = calculate_position_fit(pd.Series(p), slot)
            fit_class = fit_color_class(fit)

            st.markdown(
                f"""
                <div class="roster-card">
                    <div class="roster-slot">{slot}</div>
                    <div class="roster-player">{p['Player']}</div>
                    <div class="roster-detail">
                        {p['Team']} | Listed {p['Pos']} | {money(p['Salary'])} | BPM {float(p['BPM']):.1f} | 3P% {pct(p['3P%'])}
                    </div>
                    <div class="roster-detail">
                        Fit: <span class="{fit_class}">{fit_label(fit)} ({fit:+d})</span> — {", ".join(notes)}
                    </div>
                </div>
                """,
                unsafe_allow_html=True
            ...
            st.markdown(
                ...
                <div class="roster-card">
                    <div class="roster-slot">{slot}</div>
                    <div class="roster-player">Empty Slot</div>
                    <div class="roster-detail">Select any player. Fit will be graded after placement.</div>
                </div>
                """,
                unsafe_allow_html=True
            )
...
if len(st.session_state.roster) == 13:
    ...
    for _, row in roster_fit_df.iterrows():
        role_data.append({
            ...
            "BPM": round(row["BPM"], 1),
            "DBPM": round(row["DBPM"], 1),
            "Custom Defense": round(row["Custom_Defense"], 1),
            "Defender Score": round(row.get("Defender_Leader_Score", defender_leader_score(row)), 1),
            "Shooter Score": round(row.get("Shooter_Score", 0), 1),
            "Player Quality": round(row["Player_Quality"], 1),
            "Value Score": round(row["Value_Score"], 1),
        })

    st.dataframe(pd.DataFrame(role_data), use_container_width=True, height=455)
    ...
    st.warning(
        f"Fill all 13 roster slots to generate team results. Current roster: {len(st.session_state.roster)}/13."
    )
